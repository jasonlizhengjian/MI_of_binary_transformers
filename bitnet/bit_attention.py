...
class BitAttention(nn.Module):
    def __init__(self, dim, heads=8, mode="binary"):
        super().__init__()
        self.heads = heads
        self.scale = dim ** -0.5

        self.to_qkv = nn.ModuleList([])
        for _ in range(3):  # for Q, K, V
            self.to_qkv.append(BitLinear(dim, dim, mode=mode))
        
        self.to_out = BitLinear(dim, dim, mode=mode)

        self.attn_cache = {}  # Cache for attention pattern

    def forward(self, x, mask=None):
        b, n, _, h = *x.shape, self.heads
        qkv = [l(x).view(b, n, h, -1) for l in self.to_qkv]  # transform to Q, K, V
        q, k, v = map(lambda t: rearrange(t, 'b n h d -> b h n d'), qkv)  # rearrange Q, K, V
        ...
        self.attn_cache[f'attn_pattern_{self.__class__.__name__}'] = attn.detach()
